        ...
        let task = session.dataTask(with: url) { [weak self] data, response, error in
            ...
            guard let jsonData = data, error == nil else {
                DispatchQueue.main.async {
                    self.retrieveUsersCompletionBlock?(false, error as NSError?)
                }
                return
            }
            self.lastPage += 1
            if let users = UserViewModelController.parse(jsonData) {
                ...
                self.viewModels.append(contentsOf: newUsersPage)
                DispatchQueue.main.async {
                    self.retrieveUsersCompletionBlock?(true, nil)
                }
            } else {
                DispatchQueue.main.async {
                    self.retrieveUsersCompletionBlock?(false, NSError.createError(0, description: "JSON parsing error"))
                }
            }
        }
        task.resume()
